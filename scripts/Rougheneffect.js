/**
 * name: Roughen Effect v3g
 */

'use strict';

// =============================================================================
// ROUGHEN EFFECT v3g (Unified Multi-Effect Pipeline & Foreigner Object Extraction)
// Affinity Designer / Photo / Publisher (v3g & Multi-Effect Standard)
//
// Procedural Vector Roughen Effect:
// - Size (px / %): Roughness distortion amplitude (absolute px or relative %).
// - Detail (/in): Number of subdivisions/ridges per inch along path segments.
// - Random Seed: Configurable seed (1 - 99999) for reproducible procedural noise.
// - Smooth points:
//   * Off: Sharp corner angular peaks (lineToXY).
//   * On: Smooth wavy organic bezier curves (addBezierXY).
//
...
//   Procedural noise, segment counts, and ridge calculations are ALWAYS evaluated on the pristine S_0,
//   ensuring that segment counts, noise random seeds, and ridge positions never jump, reshuffle,
...
// - Parameter Purity:
//   Never mutates initialParams.size or initialParams.detail in the dialog.
// - 100% Style Fidelity:
...
//   Stack multiple procedural effects (Roughen, Zig Zag, Twist, Pucker & Bloat)
...
const SCRIPT_TITLE = 'Roughen Effect v3g';
const CURRENT_EFFECT_ID = 'roughen';
...
const EffectRegistry = {
  [EFFECT_ROUGHEN]: {
    ...
    sanitizeParams: function(p) {
      ...
      const seed = (p && typeof p.seed === 'number' && !isNaN(p.seed)) ? Math.max(1, Math.round(p.seed)) : 42;
...
function formatPipelineGroupName(pipeline) {
  if (!pipeline || !pipeline.length) return 'Roughen Effect';
  if (pipeline.length === 1) {
    const handler = EffectRegistry[pipeline[0].id];
    return handler ? (handler.name + ' Effect') : 'Roughen Effect';
  }
...
// =============================================================================
// GEOMETRIC SAMPLING & ROUGHEN CORE
// =============================================================================
...
// =============================================================================
// ZIG ZAG / PUCKER & BLOAT / TWIST CORES (MULTI-EFFECT ENGINE)
// =============================================================================
...
// =============================================================================
// DIALOG & USER INTERACTION (ROUGHEN EFFECT UI)
// =============================================================================
...
function showDialog(title, initialParams, stackInfo, onPreview) {
  ...
  const grp = col.addGroup('Roughen Options');

  const isRelInitial = initialParams.isRelative !== undefined ? !!initialParams.isRelative : true;
  const initialUnit = isRelInitial ? UnitType.Percent : UnitType.Point;

  const sizeEd = grp.addUnitValueEditor('Size', initialUnit, initialUnit, initialParams.size, 0, 10000);
  sizeEd.precision = 1;
  sizeEd.showPopupSlider = true;

  const relSw = grp.addSwitch('Relative', isRelInitial);

  const detailEd = grp.addUnitValueEditor('Detail (/in)', UnitType.Number, UnitType.Number, initialParams.detail, 1, 1000);
  detailEd.precision = 0;
  detailEd.showPopupSlider = true;

  const seedEd = grp.addUnitValueEditor('Random Seed', UnitType.Number, UnitType.Number, initialParams.seed, 1, 99999);
  seedEd.precision = 0;
  seedEd.showPopupSlider = true;

  const smoothSw = grp.addSwitch('Smooth points', initialParams.smooth);
  ...
  function readValues() {
    const isRel = !!relSw.value;
    return {
      enabled: activeSw ? !!activeSw.value : true,
      params: {
        size: Math.max(0, Math.round(sizeEd.value * 10) / 10),
        isRelative: isRel,
        detail: Math.max(1, Math.round(detailEd.value)),
  ...
  sizeEd.onValueChangedHandler = triggerPreview;
  relSw.onValueChangedHandler = function() {
    const isRel = !!relSw.value;
    try {
      const u = isRel ? UnitType.Percent : UnitType.Point;
      sizeEd.unit = u;
      sizeEd.displayUnit = u;
    } catch (e) {}
    triggerPreview();
  };
  detailEd.onValueChangedHandler = triggerPreview;
  seedEd.onValueChangedHandler = triggerPreview;
...
function mainUpdateMany(groups) {
  ...
  const stackInfo = {
    ...
    headerText: isEditingExisting ? '✨ Editing Roughen in Stack ✨' : '➕ Adding Roughen to Stack ➕',
  ...
  const dialogResult = showDialog(
    isEditingExisting ? 'Roughen (Edit Stack)' : 'Roughen (Add to Stack)',
    initialParams,
    stackInfo,
    function(current) {
...
module.exports.buildRoughenPolyCurve = buildRoughenPolyCurve;
