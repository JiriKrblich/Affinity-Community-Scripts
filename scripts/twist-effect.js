/**
 * name: Twist Effect v3g
 */

'use strict';

// =============================================================================
// TWIST EFFECT v3g (Unified Multi-Effect Pipeline & Foreigner Object Extraction)
// Affinity Designer / Photo / Publisher (v3g & Multi-Effect Standard)
//
// Procedural Vector Twist Effect:
// - Angle (°): Rotation twist angle (-3600° to +3600°).
// - Subdivisions: Sampling resolution per segment (4 to 100).
//
...
//   Procedural vortex twisting is ALWAYS evaluated on the pristine S_0,
//   ensuring that angular displacement and swirl geometry never jump, reshuffle,
...
// - Parameter Purity:
//   Never mutates initialParams.angle or initialParams.subdiv in the dialog.
// - 100% Style Fidelity:
...
//   Stack multiple procedural effects (Twist, Zig Zag, Roughen, Pucker & Bloat)
...
const SCRIPT_TITLE = 'Twist Effect v3g';
const CURRENT_EFFECT_ID = 'twist';
...
function formatPipelineGroupName(pipeline) {
  if (!pipeline || !pipeline.length) return 'Twist Effect';
  if (pipeline.length === 1) {
    const handler = EffectRegistry[pipeline[0].id];
    return handler ? (handler.name + ' Effect') : 'Twist Effect';
  }
...
// =============================================================================
// ROUGHEN / ZIG ZAG / PUCKER & BLOAT CORES (MULTI-EFFECT ENGINE)
// =============================================================================
...
function sampleAt(tbl, beziers, c) {
  ...
  if (a.bi === b.bi) {
    ...
    bi = Math.abs(c - a.cum) < Math.abs(c - b.cum) ? a.bi : b.bi;
    t = Math.abs(c - a.cum) < Math.abs(c - b.cum) ? a.t : b.t;
  }
...
function buildRoughenPolyCurve(sourcePolyCurve, size, isRelative, detail, smooth, seed) {
  ...
  for (const curve of sourcePolyCurve) {
    ...
    if (!pts.length) { out.addCurve(curve.clone()); continue; }
...
// =============================================================================
// DIALOG & USER INTERACTION (TWIST EFFECT UI)
// =============================================================================
...
function showDialog(title, initialParams, stackInfo, onPreview) {
  ...
  const grp = col.addGroup('Twist Options');

  const angleEd = grp.addUnitValueEditor('Angle', UnitType.Degree, UnitType.Degree, initialParams.angle, -3600, 3600);
  angleEd.precision = 1;
  angleEd.showPopupSlider = true;

  const subdivEd = grp.addUnitValueEditor('Subdivisions', UnitType.Number, UnitType.Number, initialParams.subdiv, 4, 100);
  ...
  function readValues() {
    return {
      ...
      params: {
        angle: Math.max(-3600, Math.min(3600, Math.round(angleEd.value * 10) / 10)),
        subdiv: Math.max(4, Math.min(100, Math.round(subdivEd.value)))
      }
    };
  }
  ...
  angleEd.onValueChangedHandler = triggerPreview;
  subdivEd.onValueChangedHandler = triggerPreview;
...
function mainUpdateMany(groups) {
  ...
  const stackInfo = {
    ...
    headerText: isEditingExisting ? '✨ Editing Twist in Stack ✨' : '➕ Adding Twist to Stack ➕',
  ...
  const dialogResult = showDialog(
    isEditingExisting ? 'Twist (Edit Stack)' : 'Twist (Add to Stack)',
    initialParams,
    stackInfo,
    function(current) {
...
module.exports.buildTwistPolyCurve = buildTwistPolyCurve;
