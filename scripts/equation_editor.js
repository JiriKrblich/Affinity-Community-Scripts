...
(function () {
  ...
  if (!doc) {
    app.alert("Please open a document first.", "Equation Editor");
    return;
  }
  ...
  if (editingNode) {
    try {
      const box =
        editingNode.spreadVisibleBox ||
        (typeof editingNode.getSpreadBaseBox === "function"
          ? editingNode.getSpreadBaseBox()
          : null) ||
        editingNode.baseBox;
      if (box) {
  ...
  const dlgTitle =
    mode === "A"
      ? "Edit  [" +
        initialFormula.slice(0, 36) +
        (initialFormula.length > 36 ? "…" : "") +
        "]"
      : mode === "B"
  ...
  const symColGrps = [];
  for (let c = 0; c < 5; c++)
    symColGrps.push(symStack.addColumn().addGroup(""));
  SYMBOLS.forEach((sym, i) => {
    const btn = symColGrps[i % 5].addButton(sym[0]);
    btn.setIsFullWidth(true);
    btn.setOnClickHandler(() => {
      formulaBox.text = formulaBox.text + sym[1];
    });
  });

  // ── 3. Templates ───────────────────────────────────────────────────────────
  const tmplGroup = col.addGroup("Template");
  const tmplCombo = tmplGroup.addComboBox(
    "",
    TEMPLATES.map((t) => t[0]),
    0,
  );
  tmplCombo.setIsFullWidth(true);
  const useBtn = tmplGroup.addButton(
    "Use Template ↑  (replaces formula above)",
  );
  ...
  const sizeLabel = isEditing
    ? "Size on Canvas  (preserved from original)"
    : "Size on Canvas";
  const sizeGroup = col.addGroup(sizeLabel);
  const sizeCombo = sizeGroup.addComboBox(
    "",
    SIZES.map((s) => s[1]),
    initialSizeIdx,
  );
  ...
  if (!formula) {
    app.alert("No formula entered.", "Equation Editor");
    return;
  }
  ...
  const targetH =
    isEditing && storedTargetH && !userChangedSize
      ? storedTargetH
  ...
  function parseTransformAttr(s) {
    ...
    while ((m = fnRe.exec(s)) !== null) {
      ...
      switch (fn) {
        case "matrix":
          t = [args[0], args[1], args[2], args[3], args[4] || 0, args[5] || 0];
          break;
        case "translate":
        ...
        case "scale": {
          const sx = args[0],
            sy = args.length > 1 ? args[1] : args[0];
          t = [sx, 0, 0, sy, 0, 0];
          break;
        }
        case "rotate": {
  ...
  while ((tagMatch = tagRe.exec(svgText)) !== null) {
    ...
    if (!isClose) {
      ...
      if (tagName === "g" && !selfClose) {
        ...
        if (w > 0 && h > 0)
          elements.push({ type: "rect", x, y, w, h, mat: combined });
      }
  ...
  if (elements.length === 0) {
    app.alert(
      "No drawable elements in the SVG.\nThe formula may contain unsupported commands.",
      "Equation Editor",
    );
    return;
  }
  ...
  function parseSVGPath(d) {
    const out = [],
      re = /([MmLlHhVvCcSsQqTtZz])([^MmLlHhVvCcSsQqTtZz]*)/g;
    let m;
  ...
  const pcDef = PolyCurveNodeDefinition.create(
    polyCurve,
    brushFill,
    lsd,
    noFill,
