...
// =============================================================================
// PUCKER & BLOAT EFFECT v3g (Unified Multi-Effect Pipeline & Foreigner Object Extraction)
// Affinity Designer / Photo / Publisher (v3g & Multi-Effect Standard)
//
// Procedural Vector Pucker & Bloat Effect:
// - Amount (%): Pucker (negative -1% to -200%) or Bloat (positive +1% to +200%).
//   Pulls curve handles inward/outward relative to shape center.
//
...
//   Procedural pucker/bloat calculations are ALWAYS evaluated on the pristine S_0,
//   ensuring that curvature and anchor positions never jump, reshuffle,
...
// - Parameter Purity:
//   Never mutates initialParams.amount in the dialog.
// - 100% Style Fidelity:
...
//   Stack multiple procedural effects (Pucker & Bloat, Zig Zag, Roughen, Twist)
...
function formatPipelineGroupName(pipeline) {
  if (!pipeline || !pipeline.length) return 'Pucker & Bloat';
  if (pipeline.length === 1) {
    const handler = EffectRegistry[pipeline[0].id];
    return handler ? (handler.name + ' Effect') : 'Pucker & Bloat';
  }
...
// =============================================================================
// GEOMETRIC SAMPLING & PUCKER & BLOAT CORE
// =============================================================================
...
function buildPuckerBloatPolyCurve(sourcePolyCurve, amount) {
  if (amount === 0) return sourcePolyCurve.clone();
...
// =============================================================================
// ROUGHEN / ZIG ZAG / TWIST CORES (MULTI-EFFECT ENGINE)
// =============================================================================
...
// =============================================================================
// DIALOG & USER INTERACTION (PUCKER & BLOAT EFFECT UI)
// =============================================================================
...
function showDialog(title, initialParams, stackInfo, onPreview) {
  ...
  const grp = col.addGroup('Pucker & Bloat Options');

  const amountEd = grp.addUnitValueEditor('Amount', UnitType.Percent, UnitType.Percent, initialParams.amount, -200, 200);
  amountEd.precision = 0;
  amountEd.showPopupSlider = true;
  ...
  function readValues() {
    return {
      ...
      params: {
        amount: Math.max(-200, Math.min(200, Math.round(amountEd.value)))
      }
    };
  }
  ...
  amountEd.onValueChangedHandler = triggerPreview;
...
function mainUpdateMany(groups) {
  ...
  const stackInfo = {
    ...
    headerText: isEditingExisting ? '✨ Editing Pucker & Bloat in Stack ✨' : '➕ Adding Pucker & Bloat to Stack ➕',
  ...
  const dialogResult = showDialog(
    isEditingExisting ? 'Pucker & Bloat (Edit Stack)' : 'Pucker & Bloat (Add to Stack)',
    initialParams,
    stackInfo,
    function(current) {
...
module.exports.buildPuckerBloatPolyCurve = buildPuckerBloatPolyCurve;
